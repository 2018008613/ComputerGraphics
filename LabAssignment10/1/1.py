...
def interpolateRotVec(rv1, rv2, t):
    v = np.array([0., 0., 0.])
    v[0] = lerp(rv1[0], rv2[0], t)
    v[1] = lerp(rv1[1], rv2[1], t)
    v[2] = lerp(rv1[2], rv2[2], t)
    R = exp(v)
    return R

def interpolateRotMat(R1, R2, t):
    R = np.identity(3)
    R[0][0] = lerp(R1[0][0], R2[0][0], t)
    R[0][1] = lerp(R1[0][1], R2[0][1], t)
    R[0][2] = lerp(R1[0][2], R2[0][2], t)
    R[1][0] = lerp(R1[1][0], R2[1][0], t)
    R[1][1] = lerp(R1[1][1], R2[1][1], t)
    R[1][2] = lerp(R1[1][2], R2[1][2], t)
    R[2][0] = lerp(R1[2][0], R2[2][0], t)
    R[2][1] = lerp(R1[2][1], R2[2][1], t)
    R[2][2] = lerp(R1[2][2], R2[2][2], t)
    return R

def interpolateZYXEuler(euler1, euler2, t):
    e = np.array([0.,0.,0.])
    e[0] = lerp(euler1[0], euler2[0], t)
    e[1] = lerp(euler1[1], euler2[1], t)
    e[2] = lerp(euler1[2], euler2[2], t)
